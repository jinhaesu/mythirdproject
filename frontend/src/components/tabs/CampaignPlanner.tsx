    ...
    <div className="space-y-6">
      ...
      <Card variant="bordered" className="bg-gradient-to-r from-brand/10 to-blue/10 border-brand/30">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-text-secondary mb-1">제품 URL (선택)</label>
            <div className="relative">
              <Globe size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-text-quaternary" />
              <input
                type="url"
          ...
          <div>
            <label className="block text-sm font-medium text-text-secondary mb-1">제품 설명 (선택)</label>
            <textarea
              className="w-full px-3 py-2 border border-border-primary rounded-lg text-sm resize-none"
              rows={2}
          ...
          {result.product_info?.name && (
            <Card variant="bordered">
              ...
              <div className="grid md:grid-cols-3 gap-3 text-sm">
                <div><span className="text-text-tertiary">제품명:</span> <span className="font-medium">{result.product_info.name}</span></div>
                {result.product_info.price && <div><span className="text-text-tertiary">가격:</span> <span className="font-medium">{Number(result.product_info.price).toLocaleString()}원</span></div>}
                {result.product_info.source_url && <div><span className="text-text-tertiary">URL:</span> <a href={result.product_info.source_url} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline truncate">{result.product_info.source_url}</a></div>}
              </div>
            </Card>
          )}
              ...
              {(result.campaign_structure.groups || []).map((g: any, i: number) => (
                <div key={i} className="p-3 bg-bg-0 rounded-lg">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xs bg-brand/15 text-accent-hover px-2 py-0.5 rounded">{g.budget_ratio}% | {g.budget_amount?.toLocaleString()}원</span>
                  </div>
                  <p className="text-xs text-text-tertiary">{g.reasoning}</p>
                </div>
              ))}
            </div>
          </Card>
          ...
          <Card variant="bordered">
            ...
            <div className="space-y-2">
              {(result.targeting.segments || []).map((s: any, i: number) => (
                <div key={i} className="p-3 bg-bg-0 rounded-lg flex justify-between items-start">
                  <div>
                    <span className="font-medium text-sm">{s.type}</span>
                    <p className="text-xs text-text-tertiary mt-0.5">{s.description}</p>
                    {s.interests && <div className="flex gap-1 mt-1 flex-wrap">{s.interests.map((int: string, j: number) => <span key={j} className="text-xs bg-blue/10 text-accent px-1.5 py-0.5 rounded">{int}</span>)}</div>}
                  </div>
                  <span className="text-xs text-text-tertiary whitespace-nowrap ml-2">{s.ratio}%</span>
                </div>
              ))}
            </div>
          </Card>
          ...
          <Card variant="bordered">
            ...
            <div className="space-y-3">
              {(result.copywriting.variations || []).map((v: any, i: number) => (
                <div key={i} className="p-4 bg-bg-0 rounded-lg border border-border-primary">
                  <p className="text-xs text-text-tertiary mb-2">{v.name}</p>
                  <p className="font-semibold text-base mb-1">{v.headline}</p>
                  <p className="text-sm text-text-secondary mb-1">{v.primary_text}</p>
                  <p className="text-xs text-text-tertiary">{v.description}</p>
                  <span className="inline-block mt-2 px-3 py-1 bg-brand text-white text-xs rounded-lg">{v.cta}</span>
                </div>
              ))}
            </div>
          </Card>
            ...
            <Card variant="bordered">
              ...
              <div className="space-y-2">
                {result.utm_links.map((link: any, i: number) => (
                  <div key={i} className="p-3 bg-bg-0 rounded-lg">
                    <p className="text-xs text-text-tertiary mb-1">{link.campaign} | {link.source}</p>
                    <p className="text-xs font-mono text-accent-hover break-all">{link.full_url}</p>
                  </div>
                ))}
              </div>
            </Card>
            ...
            <Card variant="bordered" className="bg-gradient-to-r from-red/10 to-bg-0 border-brand/30">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-text-tertiary">추천</span>
                </div>

                {/* 추천 이유 */}
                <p className="text-sm text-text-secondary">{result.creative_recommendation.reason}</p>
                ...
                {result.creative_recommendation.recommended_type === 'short_form_video' && result.creative_recommendation.video_plan && (
                  <div className="mt-3 p-4 bg-bg-1 rounded-lg border border-brand/30 space-y-3">
                    ...
                    <div className="space-y-2 text-sm">
                      <div>
                        <span className="text-text-tertiary">컨셉:</span>{' '}
                        <span className="font-medium">{result.creative_recommendation.video_plan.concept}</span>
                      </div>
                      <div>
                        <span className="text-text-tertiary">길이:</span>{' '}
                        <span className="font-medium">{result.creative_recommendation.video_plan.duration_seconds}초</span>
                        <span className="ml-3 text-text-tertiary">음악:</span>{' '}
                        <span className="font-medium">{result.creative_recommendation.video_plan.music_mood}</span>
                      </div>
                      <div>
                        <span className="text-text-tertiary block mb-1">스토리보드:</span>
                        <div className="space-y-1">
                          {(result.creative_recommendation.video_plan.scenes || []).map((scene: string, i: number) => (
                            <div key={i} className="flex items-start gap-2 p-2 bg-bg-0 rounded">
                              <span className="text-xs font-bold text-accent mt-0.5">#{i + 1}</span>
                              <span className="text-sm text-text-secondary">{scene}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      <div>
                        <span className="text-text-tertiary block mb-1">나레이션 스크립트:</span>
                        <p className="p-2 bg-bg-0 rounded text-sm text-text-secondary italic">
        ...
        <div className="space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-medium text-text-secondary">제품 리스트</label>
              <button onClick={addProduct} className="text-sm text-accent hover:text-accent-hover flex items-center gap-1">
                <Plus size={14} /> 추가
              </button>
            </div>
            <div className="space-y-3">
              {products.map((p, i) => (
                <div key={i} className="p-3 bg-bg-0 rounded-lg space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-text-tertiary">제품 {i + 1}</span>
                    {products.length > 1 && (
                      <button onClick={() => removeProduct(i)} className="text-text-quaternary hover:text-red">
                        <Trash2 size={14} />
                      </button>
                    )}
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    <input className="px-3 py-2 border border-border-primary rounded-lg text-sm" placeholder="제품명" value={p.name} onChange={(e) => updateProduct(i, 'name', e.target.value)} />
                    <select className="px-3 py-2 border border-border-primary rounded-lg text-sm bg-bg-1" value={p.category} onChange={(e) => updateProduct(i, 'category', e.target.value)}>
                      <option value="신제품">신제품</option>
                      <option value="주력">주력</option>
                      <option value="소진용">소진용</option>
                    </select>
                    <input className="px-3 py-2 border border-border-primary rounded-lg text-sm" type="number" placeholder="가격" value={p.price} onChange={(e) => updateProduct(i, 'price', e.target.value)} />
                  </div>
                  <input className="w-full px-3 py-2 border border-border-primary rounded-lg text-sm" placeholder="프로모션 정보 (선택)" value={p.promo_info} onChange={(e) => updateProduct(i, 'promo_info', e.target.value)} />
                </div>
              ))}
            </div>
          </div>
      ...
      <Card variant="bordered">
        ...
        {result ? (
          <div className="prose prose-sm max-w-none">
            <pre className="bg-bg-0 p-4 rounded-lg text-sm whitespace-pre-wrap overflow-auto max-h-[600px]">
    ...
    <div className="grid lg:grid-cols-2 gap-6">
      <Card variant="bordered">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-text-secondary mb-1">브랜드 정보 (선택)</label>
            <textarea className="w-full px-3 py-2 border border-border-primary rounded-lg text-sm resize-none" rows={3}
    ...
    <div className="grid lg:grid-cols-2 gap-6">
      <Card variant="bordered">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1">목적</label>
              <select className="w-full px-3 py-2 border border-border-primary rounded-lg text-sm bg-bg-1" value={purpose} onChange={(e) => setPurpose(e.target.value)}>
            ...
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1">톤</label>
              <select className="w-full px-3 py-2 border border-border-primary rounded-lg text-sm bg-bg-1" value={tone} onChange={(e) => setTone(e.target.value)}>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-medium text-text-secondary">제품 정보</label>
              <button onClick={addProduct} className="text-sm text-accent hover:text-accent-hover flex items-center gap-1">
                <Plus size={14} /> 추가
              </button>
            </div>
            {products.map((p, i) => (
              <div key={i} className="p-3 bg-bg-0 rounded-lg space-y-2 mb-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-text-tertiary">제품 {i + 1}</span>
                  {products.length > 1 && <button onClick={() => removeProduct(i)} className="text-text-quaternary hover:text-red"><Trash2 size={14} /></button>}
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <input className="px-3 py-2 border border-border-primary rounded-lg text-sm" placeholder="제품명" value={p.name} onChange={(e) => updateProduct(i, 'name', e.target.value)} />
                  <input className="px-3 py-2 border border-border-primary rounded-lg text-sm" type="number" placeholder="가격" value={p.price} onChange={(e) => updateProduct(i, 'price', e.target.value)} />
                </div>
                <input className="w-full px-3 py-2 border border-border-primary rounded-lg text-sm" placeholder="제품 설명" value={p.description} onChange={(e) => updateProduct(i, 'description', e.target.value)} />
                <input className="w-full px-3 py-2 border border-border-primary rounded-lg text-sm" placeholder="프로모션 (선택)" value={p.promo} onChange={(e) => updateProduct(i, 'promo', e.target.value)} />
              </div>
            ))}
          </div>
    ...
    <div className="grid lg:grid-cols-2 gap-6">
      <Card variant="bordered">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-text-secondary mb-2">플랫폼</label>
            <div className="flex flex-wrap gap-2">
              {['facebook', 'instagram', 'google', 'naver', 'kakao'].map((p) => (
                <button key={p} onClick={() => togglePlatform(p)}
                  className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                    platforms.includes(p) ? 'bg-brand text-white' : 'bg-bg-2 text-text-tertiary hover:bg-bg-2/7'
                  }`}>
                  {p}
                </button>
              ))}
        ...
        {result ? (
          <div className="space-y-2 max-h-[600px] overflow-auto">
            {Array.isArray(result) ? result.map((item: any, i: number) => (
              <div key={i} className="p-3 bg-bg-0 rounded-lg">
                <p className="text-xs text-text-tertiary mb-1">{item.campaign || item.label || `링크 ${i + 1}`}</p>
                <p className="text-sm text-accent-hover break-all font-mono">{item.url || item.utm_url || JSON.stringify(item)}</p>
              </div>
            )) : (
              <pre className="bg-bg-0 p-4 rounded-lg text-sm whitespace-pre-wrap">{JSON.stringify(result, null, 2)}</pre>
            )}
          </div>
        ) : (
          <div className="text-center py-16 text-text-quaternary">
    ...
    <div className="grid lg:grid-cols-2 gap-6">
      <Card variant="bordered">
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="border-2 border-dashed border-border-primary rounded-lg p-6 text-center hover:border-brand transition-colors">
              ...
              <label htmlFor="csv-upload" className="cursor-pointer">
                ...
                <p className="text-sm text-text-tertiary">{file ? file.name : '클릭하여 CSV 파일 선택'}</p>
                <p className="text-xs text-text-quaternary mt-1">Meta, Google, Naver 광고 데이터</p>
              </label>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1">플랫폼</label>
              <select className="w-full px-3 py-2 border border-border-primary rounded-lg text-sm bg-bg-1" value={platform} onChange={(e) => setPlatform(e.target.value)}>
            ...
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1">분석 유형</label>
              <select className="w-full px-3 py-2 border border-border-primary rounded-lg text-sm bg-bg-1" value={analysisType} onChange={(e) => setAnalysisType(e.target.value)}>
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-6">
      <Card variant="bordered">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-text-secondary mb-1">새 소재 설명</label>
            <textarea className="w-full px-3 py-2 border border-border-primary rounded-lg text-sm resize-none" rows={3}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-medium text-text-secondary">과거 소재 성과</label>
              <button onClick={addPast} className="text-sm text-accent hover:text-accent-hover flex items-center gap-1">
                <Plus size={14} /> 추가
              </button>
            </div>
            {pastCreatives.map((p, i) => (
              <div key={i} className="p-3 bg-bg-0 rounded-lg space-y-2 mb-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-text-tertiary">과거 소재 {i + 1}</span>
                  {pastCreatives.length > 1 && <button onClick={() => removePast(i)} className="text-text-quaternary hover:text-red"><Trash2 size={14} /></button>}
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <select className="px-3 py-2 border border-border-primary rounded-lg text-sm bg-bg-1" value={p.type} onChange={(e) => updatePast(i, 'type', e.target.value)}>
                    <option value="IMAGE">이미지</option>
                    <option value="VIDEO">영상</option>
                    <option value="CAROUSEL">캐러셀</option>
                  </select>
                  <input className="px-3 py-2 border border-border-primary rounded-lg text-sm" placeholder="스타일" value={p.style} onChange={(e) => updatePast(i, 'style', e.target.value)} />
                </div>
                <div className="grid grid-cols-3 gap-2">
                  <input className="px-3 py-2 border border-border-primary rounded-lg text-sm" type="number" step="0.01" placeholder="CTR (%)" value={p.ctr} onChange={(e) => updatePast(i, 'ctr', e.target.value)} />
                  <input className="px-3 py-2 border border-border-primary rounded-lg text-sm" type="number" step="0.01" placeholder="CVR (%)" value={p.cvr} onChange={(e) => updatePast(i, 'cvr', e.target.value)} />
                  <input className="px-3 py-2 border border-border-primary rounded-lg text-sm" type="number" placeholder="지출액" value={p.spend} onChange={(e) => updatePast(i, 'spend', e.target.value)} />
                </div>
              </div>
            ))}
          </div>
