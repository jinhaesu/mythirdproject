      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-text-primary flex items-center gap-2">
            <FileText className="text-green" size={28} />
            네이버 리포트
          </h1>
          <p className="text-sm text-text-tertiary mt-1">검색광고 / GFA 통합 리포트 생성</p>
        </div>
        <div className="flex items-center bg-bg-2 rounded-lg p-0.5">
          <button
            onClick={() => setActiveSection('generate')}
            className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
              activeSection === 'generate' ? 'bg-bg-1 shadow-[0px_1px_3px_rgba(0,0,0,0.2)] text-green' : 'text-text-tertiary hover:text-text-secondary'
            }`}
          >
            리포트 생성
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
              activeSection === 'schedules' ? 'bg-bg-1 shadow-[0px_1px_3px_rgba(0,0,0,0.2)] text-green' : 'text-text-tertiary hover:text-text-secondary'
            }`}
          >
            스케줄 리포트
          </button>
      ...
      {activeSection === 'generate' && (
        <div className="space-y-6">
          ...
          <div className="bg-bg-1 rounded-xl border border-border-primary p-6">
            <h2 className="text-base font-semibold text-text-primary mb-4">리포트 유형</h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {REPORT_TYPES.map((type) => {
                ...
                return (
                  ...
                  >
                    <Icon size={24} className={reportForm.type === type.value ? 'text-green' : 'text-text-quaternary'} />
                    <p className="text-sm font-medium text-text-primary mt-2">{type.label}</p>
          ...
          {/* Date Range & Format */}
          <div className="bg-bg-1 rounded-xl border border-border-primary p-6">
            <h2 className="text-base font-semibold text-text-primary mb-4">기간 및 형식</h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
              ...
              <div>
                <label className="block text-sm font-medium text-text-secondary mb-1">분석 수준</label>
                <select
          ...
          {/* Metrics Selection */}
          <div className="bg-bg-1 rounded-xl border border-border-primary p-6">
            <h2 className="text-base font-semibold text-text-primary mb-4">지표 선택</h2>
          ...
          {/* Actions */}
          <div className="bg-bg-1 rounded-xl border border-border-primary p-6">
            <h2 className="text-base font-semibold text-text-primary mb-4">리포트 생성</h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {/* PDF Download */}
              <button
                onClick={() => {
                ...
                className="flex items-center justify-center gap-2 p-4 border border-border-primary rounded-lg text-sm font-medium text-text-secondary hover:bg-bg-2/5 transition-colors disabled:opacity-50"
              >
          ...
          {showScheduleForm && (
            <div className="bg-bg-1 rounded-xl border border-border-primary p-6">
              ...
              <div className="max-w-lg space-y-4">
                <div>
                  <label className="block text-sm font-medium text-text-secondary mb-1">스케줄명</label>
                  <input
                    type="text"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-text-secondary mb-1">리포트 유형</label>
                    <select
                  ...
                  <div>
                    <label className="block text-sm font-medium text-text-secondary mb-1">발송 주기</label>
                    <select
                ...
                {scheduleForm.frequency === 'weekly' && (
                  <div>
                    <label className="block text-sm font-medium text-text-secondary mb-1">요일</label>
                    <select
                ...
                <div>
                  <label className="block text-sm font-medium text-text-secondary mb-1">수신 이메일</label>
                  <input
                    type="email"
                ...
                <div>
                  <label className="block text-sm font-medium text-text-secondary mb-2">포함 지표</label>
                  <div className="flex flex-wrap gap-2">
          ...
          <div className="bg-bg-1 rounded-xl border border-border-primary overflow-hidden">
            ...
            {loadingSchedules ? (
              ...
              <div className="text-center py-12 text-text-tertiary">
                <Calendar size={48} className="mx-auto mb-3 text-text-quaternary" />
                <p>등록된 스케줄이 없습니다.</p>
                <button
              ...
              <div className="divide-y divide-border-primary">
                {schedules.map((sched: any) => {
                  ...
                  return (
                    <div key={schedId} className="px-6 py-4 hover:bg-bg-2/5 flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-text-primary">{sched.name}</p>
                        <div className="flex items-center gap-3 mt-1 text-xs text-text-tertiary flex-wrap gap-y-1">
                          <span className="px-1.5 py-0.5 bg-green/10 text-green rounded">{typeLabel}</span>
