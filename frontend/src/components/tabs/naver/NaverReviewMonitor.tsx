...
function ReviewAiAnalysis({ text }: { text: string }) {
  ...
  const detectStyle = (t: string) => {
    ...
    if (l.includes('불만') || l.includes('이슈') || l.includes('top')) return { icon: '🔴', color: 'border-red/30 bg-red/10' };
    if (l.includes('긴급') || l.includes('대응')) return { icon: '⚡', color: 'border-orange/30 bg-orange/10' };
    if (l.includes('전략') || l.includes('개선')) return { icon: '🛠️', color: 'border-brand/30 bg-blue/10' };
    if (l.includes('긍정')) return { icon: '💚', color: 'border-green/30 bg-green/10' };
    return { icon: '📋', color: 'border-border-primary bg-bg-0' };
  };
...
export function NaverReviewMonitor() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-text-primary flex items-center gap-2"><MessageSquare className="text-green" size={28} /> 리뷰 모니터링</h1>
        <p className="text-sm text-text-tertiary mt-1">네이버 쇼핑 제품 리뷰 수집 · 별점 분석 · AI 이슈 진단</p>
      </div>
      ...
      <div className="bg-bg-1 rounded-xl border border-border-primary p-5">
        <h2 className="text-sm font-semibold text-text-primary mb-3">모니터링 제품 ({products.length})</h2>
        {products.length === 0 ? <p className="text-xs text-text-quaternary py-8 text-center">등록된 제품이 없습니다.</p> : (
          <div className="space-y-2">{products.map((p: any) => (
            <div key={p.id} onClick={() => { setSelectedProductId(p.id); setAnalysisResult(null); }}
              className={clsx('flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-colors', selectedProductId === p.id ? 'border-green bg-green/10' : 'border-border-primary hover:bg-bg-2/5')}>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-green/15 rounded-lg flex items-center justify-center text-green"><MessageSquare size={16} /></div>
                <div><p className="text-sm font-medium text-text-primary">{p.product_name}</p><p className="text-xs text-text-quaternary truncate max-w-xs">{p.product_url}</p></div>
              </div>
              <div className="flex items-center gap-2">
                <a href={p.product_url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="p-1.5 text-text-quaternary hover:text-accent"><ExternalLink size={14} /></a>
                <button onClick={(e) => { e.stopPropagation(); deleteMutation.mutate(p.id); }} className="p-1.5 text-text-quaternary hover:text-red"><Trash2 size={14} /></button>
              </div>
            </div>
          ))}</div>
        )}
      ...
      {selectedProduct && (
        <div className="bg-bg-1 rounded-xl border border-border-primary p-5 space-y-4">
            ...
            <div className="space-y-5">
              ...
              <div className="bg-bg-0 rounded-xl p-4 border">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">{[
                  { label: '7일', count: analysisResult.stats.low_star_count_7d, color: 'text-red' },
                  { label: '14일', count: analysisResult.stats.low_star_count_14d, color: 'text-orange' },
                  { label: '30일', count: analysisResult.stats.low_star_count_30d, color: 'text-yellow' },
                ].map((item, i) => <div key={i} className="bg-bg-1 rounded-lg p-3 text-center border"><p className={clsx('text-xl font-bold', item.color)}>{item.count}건</p><p className="text-xs text-text-tertiary">{item.label} 이내</p></div>)}</div>
              </div>
              <div className="bg-bg-0 rounded-xl p-4 border">
                <h3 className="text-sm font-semibold text-text-primary mb-3 flex items-center gap-1.5"><BarChart3 size={14} /> 별점 분포</h3>
              ...
              {analysisResult.stats.low_reviews_sample?.length > 0 && (
                <div className="bg-bg-0 rounded-xl p-4 border">
                  <h3 className="text-sm font-semibold text-text-primary mb-3 flex items-center gap-1.5"><AlertTriangle size={14} className="text-red" /> 저별점 리뷰</h3>
                  <div className="space-y-2 max-h-64 overflow-y-auto">{analysisResult.stats.low_reviews_sample.map((r: any, idx: number) => (
                    <div key={idx} className="bg-bg-1 rounded-lg p-3 border text-xs">
                      <div className="flex items-center gap-2 mb-1"><Stars rating={r.rating} size={11} /><span className="text-text-tertiary">{r.writer}</span><span className="text-text-quaternary ml-auto">{r.date?.slice(0, 10)}</span></div>
                      <p className="text-text-secondary leading-relaxed">{r.content?.slice(0, 200)}</p>
                    </div>
                  ))}</div>
                </div>
              )}
  ...
  return (
    <div className="bg-bg-1 rounded-xl border border-border-primary p-5">
      ...
      {schedules.length > 0 ? <div className="space-y-2">{schedules.map((s: any) => (
        <div key={s.id} className="flex items-center justify-between p-3 bg-bg-0 rounded-lg border text-xs">
          <div className="flex items-center gap-2 flex-wrap"><span className="font-medium text-text-primary">{s.name}</span><span className="text-text-quaternary">|</span><span className="text-text-tertiary">{(s.days_of_week||[]).map((d:number)=>['일','월','화','수','목','금','토'][d]).join('·')} {String(s.send_hour??9).padStart(2,'0')}:{String(s.send_minute??0).padStart(2,'0')}</span><span className="text-text-quaternary">&rarr;</span><span className="text-green">{s.email_to}</span><span className="px-1.5 py-0.5 bg-red/15 text-red rounded text-[10px]">{s.star_threshold}점↓</span></div>
          <div className="flex gap-1.5 ml-2"><button onClick={() => runNow.mutate(s.id)} disabled={runNow.isPending} className="p-1.5 bg-green/10 text-green rounded hover:bg-green/15" title="즉시 실행">{runNow.isPending ? <Loader2 size={13} className="animate-spin" /> : <Star size={13} />}</button><button onClick={() => del.mutate(s.id)} className="p-1.5 bg-red/10 text-red rounded hover:bg-red/15" title="삭제"><Trash2 size={13} /></button></div>
        </div>
      ))}</div> : <p className="text-xs text-text-quaternary py-2">등록된 스케줄이 없습니다.</p>}
    </div>
  );
}
