...
import {
  ...
  BarChart3, Sparkles, ExternalLink, MessageSquare, Upload, ClipboardPaste,
} from 'lucide-react';
...
const reviewApi = {
  ...
  analyzeReviews: async (productName: string, reviews: any[], starThreshold: number) => {
    const { data } = await api.post('/naver/review-monitor/analyze-reviews', { product_name: productName, reviews, star_threshold: starThreshold });
    return data;
  },
...
// ── 리뷰 파싱 (탭/줄바꿈으로 구분된 텍스트 또는 CSV) ──
function parseReviewText(text: string): any[] {
  const lines = text.trim().split('\n').filter(l => l.trim());
  const reviews: any[] = [];
  for (const line of lines) {
    // TSV/CSV 형식: 별점\t내용\t날짜 또는 별점,내용,날짜
    const parts = line.includes('\t') ? line.split('\t') : line.split(',');
    if (parts.length >= 2) {
      const rating = parseInt(parts[0].trim());
      if (rating >= 1 && rating <= 5) {
        reviews.push({ rating, content: parts.slice(1).join(' ').trim(), date: '', writer: '익명' });
        continue;
      }
    }
    // 별점이 숫자로 시작하는 경우: "5 맛있어요 잘 먹겠습니다"
    const m = line.match(/^(\d)\s+(.+)/);
    if (m) {
      reviews.push({ rating: parseInt(m[1]), content: m[2].trim(), date: '', writer: '익명' });
      continue;
    }
    // 별점 없이 텍스트만 있으면 5점으로 처리
    if (line.trim().length > 5) {
      reviews.push({ rating: 5, content: line.trim(), date: '', writer: '익명' });
    }
  }
  return reviews;
}
...
export function NaverReviewMonitor() {
  ...
  const analyzeMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!product) throw new Error('제품을 선택해주세요.');

      const reviews = parseReviewText(reviewText);
      if (!reviews.length) throw new Error('리뷰 데이터를 입력해주세요.');

      return await reviewApi.analyzeReviews(product.product_name, reviews, starThreshold);
    },
    onSuccess: (data) => { setAnalysisResult(data); if (data?.stats?.total_reviews > 0) toast.success('리뷰 분석 완료!'); },
    onError: (e: any) => toast.error(e?.message || '분석 실패'),
  });
  ...
  const handleCsvUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => { setReviewText(ev.target?.result as string || ''); toast.success('파일 로드 완료'); };
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
          <MessageSquare className="text-green-600" size={28} /> 리뷰 모니터링
        </h1>
        <p className="text-sm text-gray-500 mt-1">제품 리뷰 데이터를 붙여넣기/CSV 업로드하여 AI로 분석합니다</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-3">
        ...
        <div className="grid sm:grid-cols-[1fr_2fr_auto] gap-2">
          ...
          <input type="text" value={productUrl} onChange={(e) => setProductUrl(e.target.value)} placeholder="네이버 쇼핑 제품 URL (참고용)" className="px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-green-500" />
          <button onClick={() => registerMutation.mutate()} disabled={!productName.trim() || registerMutation.isPending} className="flex items-center gap-1.5 px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 disabled:opacity-50">
            <Plus size={15} /> 등록
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        ...
        {products.length === 0 ? (
          <p className="text-xs text-gray-400 py-8 text-center">등록된 제품이 없습니다.</p>
        ) : (
          <div className="space-y-2">
            {products.map((p: any) => (
              <div key={p.id} onClick={() => { setSelectedProductId(p.id); setAnalysisResult(null); setShowPasteArea(true); }}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-sm font-medium text-gray-900">{p.product_name}</p>{p.product_url && <p className="text-xs text-gray-400 truncate max-w-xs">{p.product_url}</p>}</div>
                </div>
                <div className="flex items-center gap-2">
                  {p.product_url && <a href={p.product_url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="p-1.5 text-gray-400 hover:text-blue-600"><ExternalLink size={14} /></a>}
      ...
      {selectedProduct && (
        <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <select value={starThreshold} onChange={(e) => setStarThreshold(Number(e.target.value))} className="px-2 py-1 border rounded text-xs">
                {[1, 2, 3, 4].map(n => <option key={n} value={n}>{n}점 이하 기준</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="p-3 bg-blue-50 rounded-lg border border-blue-100 text-xs text-blue-800 space-y-1">
            <p className="font-semibold">리뷰 데이터 입력 방법:</p>
            <p>1. 스마트스토어 판매자센터 → 리뷰 관리 → 리뷰 목록을 복사</p>
            <p>2. 아래 입력창에 붙여넣기 (별점과 내용을 탭/쉼표로 구분)</p>
            <p>3. 또는 CSV 파일 업로드 (별점,리뷰내용 형식)</p>
            <p className="text-blue-600 mt-1">예시: <code className="bg-blue-100 px-1 rounded">5	맛있어요 잘 먹겠습니다</code> 또는 <code className="bg-blue-100 px-1 rounded">3,배송이 좀 느렸어요</code></p>
          </div>

          {/* 입력 영역 */}
          <div className="space-y-2">
            <div className="flex gap-2">
              <button onClick={() => setShowPasteArea(!showPasteArea)} className={clsx('flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border', showPasteArea ? 'bg-green-50 text-green-700 border-green-200' : 'text-gray-600 border-gray-200 hover:bg-gray-50')}>
                <ClipboardPaste size={13} /> 붙여넣기
              </button>
              <label className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-600 rounded-lg border border-gray-200 hover:bg-gray-50 cursor-pointer">
                <Upload size={13} /> CSV 업로드
                <input type="file" accept=".csv,.txt,.tsv" onChange={handleCsvUpload} className="hidden" />
              </label>
              {reviewText && <span className="text-xs text-gray-400 self-center">{parseReviewText(reviewText).length}건 파싱됨</span>}
            </div>
            ...
            {showPasteArea && (
              <textarea
                ...
                placeholder={"별점\t리뷰내용\n5\t맛있어요 정말 맛있습니다\n3\t배송이 늦었어요\n1\t상품이 파손되어 왔습니다"}
                rows={8}
                className="w-full px-3 py-2 border rounded-lg text-xs font-mono focus:outline-none focus:ring-1 focus:ring-green-500 resize-y"
              />
            )}
          </div>
          ...
          <button
            ...
            disabled={!reviewText.trim() || analyzeMutation.isPending}
            className="flex items-center gap-1.5 px-5 py-2.5 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 disabled:opacity-50"
          >
            {analyzeMutation.isPending ? <><Loader2 size={14} className="animate-spin" /> 분석 중...</> : <><Sparkles size={14} /> AI 리뷰 분석</>}
          </button>

          {/* 분석 결과 */}
          {analysisResult?.error && (
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-800">{analysisResult.error}</div>
          )}
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="p-3 bg-yellow-50 rounded-xl border border-yellow-100 text-center">
                  ...
                  <p className="text-xs text-yellow-600 mt-0.5 flex items-center justify-center gap-1"><Stars rating={Math.round(analysisResult.stats.average_rating)} size={10} /> 평균</p>
                </div>
                <div className="p-3 bg-red-50 rounded-xl border border-red-100 text-center">
                  <p className="text-2xl font-bold text-red-700">{analysisResult.stats.low_star_total}</p>
                  <p className="text-xs text-red-600 mt-0.5">{starThreshold}점 이하</p>
                </div>
                <div className="p-3 bg-green-50 rounded-xl border border-green-100 text-center">
                  <p className="text-2xl font-bold text-green-700">{analysisResult.stats.total_reviews - analysisResult.stats.low_star_total}</p>
                  <p className="text-xs text-green-600 mt-0.5">{starThreshold + 1}점 이상</p>
                </div>
              </div>
                    ...
                    return (
                      <div key={star} className="flex items-center gap-2">
                        ...
                        <div className="flex-1 h-5 bg-gray-200 rounded-full overflow-hidden">
                          <div className={clsx('h-full rounded-full', star >= 4 ? 'bg-green-500' : star === 3 ? 'bg-yellow-500' : 'bg-red-500')} style={{ width: `${pct}%` }} />
                        </div>
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    {analysisResult.stats.low_reviews_sample.map((r: any, idx: number) => (
                      <div key={idx} className="bg-white rounded-lg p-3 border text-xs">
                        <div className="flex items-center gap-2 mb-1"><Stars rating={r.rating} size={11} /><span className="text-gray-500">{r.writer}</span></div>
...
function ReviewSchedulePanel() {
  ...
  const runNow = useMutation({ mutationFn: (id: string) => reviewApi.runScheduleNow(id), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['review-schedules'] }); toast.success('실행 완료'); }, onError: (e: any) => toast.error(e?.response?.data?.detail || '실행 실패') });
  const DOW = ['월','화','수','목','금','토','일'];

  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-semibold text-gray-800 flex items-center gap-1.5"><BarChart3 size={15} /> 정기 리뷰 리포트 (이메일)</h2>
        <button onClick={() => setShowForm(!showForm)} className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-green-700 bg-green-50 rounded-lg hover:bg-green-100 border border-green-200"><Plus size={13} /> 스케줄 추가</button>
      </div>
      {showForm && (
        <div className="p-4 bg-gray-50 rounded-lg border space-y-4 mb-4">
          <div>
            ...
            <div className="flex gap-1.5">{DOW.map((label, idx) => { const val = idx < 5 ? idx + 1 : idx === 5 ? 6 : 0; return (<button key={idx} onClick={() => toggleDay(val)} className={clsx('w-10 h-10 rounded-lg text-sm font-medium transition-all', days.includes(val) ? 'bg-green-600 text-white shadow-sm' : 'bg-white text-gray-500 border hover:border-green-300')}>{label}</button>); })}</div>
          </div>
          <div className="flex items-center gap-4">
            <div>
              <label className="text-xs text-gray-500 mb-1 block">시간 (KST)</label>
              <div className="flex items-center gap-1">
                <select value={hour} onChange={(e) => setHour(Number(e.target.value))} className="px-2 py-1.5 border rounded-lg text-sm w-20">{Array.from({ length: 24 }, (_, i) => <option key={i} value={i}>{String(i).padStart(2, '0')}시</option>)}</select>
                <span className="text-gray-400 font-bold">:</span>
                <select value={minute} onChange={(e) => setMinute(Number(e.target.value))} className="px-2 py-1.5 border rounded-lg text-sm w-20">{Array.from({ length: 60 }, (_, i) => <option key={i} value={i}>{String(i).padStart(2, '0')}분</option>)}</select>
              </div>
            </div>
            <div>
              <label className="text-xs text-gray-500 mb-1 block">별점 기준</label>
              <select value={threshold} onChange={(e) => setThreshold(Number(e.target.value))} className="px-2 py-1.5 border rounded-lg text-sm">{[1, 2, 3, 4].map(n => <option key={n} value={n}>{n}점 이하</option>)}</select>
            </div>
          </div>
          <div><label className="text-xs text-gray-500 mb-1 block">수신 이메일</label><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="report@example.com" className="w-full px-3 py-1.5 border rounded-lg text-sm" /></div>
          <div className="flex gap-2">
            <button onClick={() => create.mutate()} disabled={!email || days.length === 0 || create.isPending} className="px-4 py-1.5 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 disabled:opacity-50">{create.isPending ? '등록 중...' : '스케줄 등록'}</button>
      ...
      {schedules.length > 0 ? (
        <div className="space-y-2">{schedules.map((s: any) => (
          <div key={s.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border text-xs">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="font-medium text-gray-900">{s.name}</span><span className="text-gray-300">|</span>
              <span className="text-gray-600">{(s.days_of_week || []).map((d: number) => ['일','월','화','수','목','금','토'][d]).join('·')} {String(s.send_hour ?? 9).padStart(2, '0')}:{String(s.send_minute ?? 0).padStart(2, '0')}</span>
              <span className="text-gray-300">&rarr;</span><span className="text-green-700">{s.email_to}</span>
              <span className="px-1.5 py-0.5 bg-red-100 text-red-700 rounded text-[10px]">{s.star_threshold}점↓</span>
            </div>
            <div className="flex gap-1.5 ml-2">
              <button onClick={() => runNow.mutate(s.id)} disabled={runNow.isPending} className="p-1.5 bg-green-50 text-green-700 rounded hover:bg-green-100" title="즉시 실행">{runNow.isPending ? <Loader2 size={13} className="animate-spin" /> : <Star size={13} />}</button>
        ...
        <p className="text-xs text-gray-400 py-2">등록된 스케줄이 없습니다.</p>
      )}
    </div>
  );
}
