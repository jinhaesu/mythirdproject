...
function Stars({ rating, size = 14 }: { rating: number; size?: number }) {
  return <span className="inline-flex gap-0.5">{[1,2,3,4,5].map(n => <Star key={n} size={size} className={n <= rating ? 'text-yellow-400 fill-yellow-400' : 'text-[#62666D]'} />)}</span>;
}
...
function ReviewAiAnalysis({ text }: { text: string }) {
  ...
  const detectStyle = (t: string) => {
    ...
    if (l.includes('불만') || l.includes('이슈') || l.includes('top')) return { icon: '🔴', color: 'border-[#EB5757]/30 bg-[#EB5757]/10' };
    if (l.includes('긴급') || l.includes('대응')) return { icon: '⚡', color: 'border-[#FC7840]/30 bg-[#FC7840]/10' };
    if (l.includes('전략') || l.includes('개선')) return { icon: '🛠️', color: 'border-[#5E6AD2]/30 bg-[#4EA7FC]/10' };
    if (l.includes('긍정')) return { icon: '💚', color: 'border-[#27A644]/30 bg-[#27A644]/10' };
    return { icon: '📋', color: 'border-[#23252A] bg-[#08090A]' };
  };
  ...
  flush();
  if (sections.length <= 1) return <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-200"><h4 className="text-sm font-semibold text-emerald-900 mb-2 flex items-center gap-1.5"><Sparkles size={14} /> AI 리뷰 분석</h4><div className="text-xs text-[#D0D6E0] whitespace-pre-line leading-relaxed">{text}</div></div>;
  return (
    <div className="space-y-3">
      <h4 className="text-sm font-semibold text-[#F7F8F8] flex items-center gap-1.5"><Sparkles size={15} /> AI 리뷰 분석</h4>
      <div className="grid gap-3">{sections.map((sec, i) => (
        <div key={i} className={clsx('rounded-xl border p-4', sec.color)}>
          {sec.title && <div className="flex items-center gap-2 mb-2"><span className="text-base">{sec.icon}</span><h5 className="text-sm font-semibold text-[#F7F8F8]">{sec.title}</h5></div>}
          <div className="text-xs text-[#D0D6E0] leading-relaxed whitespace-pre-line">{sec.content.split('\n').map((l, j) => { const t = l.trim(); if (!t) return null; const bm = t.match(/^[-•▸→]\s*(.+)/); return bm ? <div key={j} className="flex gap-1.5 py-0.5"><span className="text-green-500">▸</span><span dangerouslySetInnerHTML={{ __html: bm[1].replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>') }} /></div> : <p key={j} className="py-0.5" dangerouslySetInnerHTML={{ __html: t.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>') }} />; })}</div>
        </div>
      ))}</div>
    </div>
  );
...
export function NaverReviewMonitor() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[#F7F8F8] flex items-center gap-2"><MessageSquare className="text-[#27A644]" size={28} /> 리뷰 모니터링</h1>
        <p className="text-sm text-[#8A8F98] mt-1">네이버 쇼핑 제품 리뷰 수집 · 별점 분석 · AI 이슈 진단</p>
      </div>

      <div className="bg-[#0F1011] rounded-xl border border-[#23252A] p-5 space-y-3">
        <h2 className="text-sm font-semibold text-[#F7F8F8]">제품 등록</h2>
        <div className="grid sm:grid-cols-[1fr_2fr_auto] gap-2">
          ...
          <button onClick={() => registerMutation.mutate()} disabled={!productName.trim() || !productUrl.trim() || registerMutation.isPending} className="flex items-center gap-1.5 px-4 py-2 bg-[#27A644] text-white text-sm font-medium rounded-lg hover:bg-green-700 disabled:opacity-50"><Plus size={15} /> 등록</button>
        </div>
      </div>

      <div className="bg-[#0F1011] rounded-xl border border-[#23252A] p-5">
        <h2 className="text-sm font-semibold text-[#F7F8F8] mb-3">모니터링 제품 ({products.length})</h2>
        {products.length === 0 ? <p className="text-xs text-[#62666D] py-8 text-center">등록된 제품이 없습니다.</p> : (
          <div className="space-y-2">{products.map((p: any) => (
            <div key={p.id} onClick={() => { setSelectedProductId(p.id); setAnalysisResult(null); }}
              className={clsx('flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-colors', selectedProductId === p.id ? 'border-[#27A644] bg-[#27A644]/10' : 'border-[#23252A] hover:bg-[#141516]/5')}>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-[#27A644]/15 rounded-lg flex items-center justify-center text-[#27A644]"><MessageSquare size={16} /></div>
                <div><p className="text-sm font-medium text-[#F7F8F8]">{p.product_name}</p><p className="text-xs text-[#62666D] truncate max-w-xs">{p.product_url}</p></div>
              </div>
              <div className="flex items-center gap-2">
                <a href={p.product_url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="p-1.5 text-[#62666D] hover:text-[#7070FF]"><ExternalLink size={14} /></a>
                <button onClick={(e) => { e.stopPropagation(); deleteMutation.mutate(p.id); }} className="p-1.5 text-[#62666D] hover:text-[#EB5757]"><Trash2 size={14} /></button>
              </div>
            </div>
          ))}</div>
        )}
        ...
        <div className="bg-[#0F1011] rounded-xl border border-[#23252A] p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => analyzeMutation.mutate()} disabled={analyzeMutation.isPending} className="flex items-center gap-1.5 px-4 py-2 bg-[#27A644] text-white text-sm font-medium rounded-lg hover:bg-green-700 disabled:opacity-50">
          ...
          {analysisResult?.error && (
            <div className="p-3 bg-[#F0BF00]/10 border border-[#F0BF00]/30 rounded-lg text-xs text-amber-800">
          ...
          {analysisResult?.stats && analysisResult.stats.total_reviews > 0 && (
            <div className="space-y-5">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="p-3 bg-[#4EA7FC]/10 rounded-xl border border-blue-100 text-center"><p className="text-2xl font-bold text-[#828FFF]">{analysisResult.stats.total_reviews.toLocaleString()}</p><p className="text-xs text-[#7070FF] mt-0.5">전체 리뷰</p></div>
                <div className="p-3 bg-[#F0BF00]/10 rounded-xl border border-yellow-100 text-center"><p className="text-2xl font-bold text-[#F0BF00]">{analysisResult.stats.average_rating}</p><p className="text-xs text-[#F0BF00] mt-0.5 flex items-center justify-center gap-1"><Stars rating={Math.round(analysisResult.stats.average_rating)} size={10} /> 평균</p></div>
                <div className="p-3 bg-[#EB5757]/10 rounded-xl border border-red-100 text-center"><p className="text-2xl font-bold text-[#EB5757]">{analysisResult.stats.low_star_total}</p><p className="text-xs text-[#EB5757] mt-0.5">{starThreshold}점 이하</p></div>
                <div className="p-3 bg-[#FC7840]/10 rounded-xl border border-orange-100 text-center"><p className="text-2xl font-bold text-[#FC7840]">{analysisResult.stats.low_star_count_7d}</p><p className="text-xs text-[#FC7840] mt-0.5">최근 7일</p></div>
              </div>
              <div className="bg-[#08090A] rounded-xl p-4 border">
                ...
                ].map((item, i) => <div key={i} className="bg-[#0F1011] rounded-lg p-3 text-center border"><p className={clsx('text-xl font-bold', item.color)}>{item.count}건</p><p className="text-xs text-[#8A8F98]">{item.label} 이내</p></div>)}</div>
              </div>
              <div className="bg-[#08090A] rounded-xl p-4 border">
                <h3 className="text-sm font-semibold text-[#F7F8F8] mb-3 flex items-center gap-1.5"><BarChart3 size={14} /> 별점 분포</h3>
                <div className="space-y-2">{[5,4,3,2,1].map(star => {
                  ...
                  return <div key={star} className="flex items-center gap-2"><span className="text-xs text-[#8A8F98] w-12 flex items-center gap-0.5"><Star size={11} className="text-yellow-400 fill-yellow-400" /> {star}점</span><div className="flex-1 h-5 bg-[#232326] rounded-full overflow-hidden"><div className={clsx('h-full rounded-full', star >= 4 ? 'bg-[#27A644]' : star === 3 ? 'bg-[#F0BF00]' : 'bg-[#EB5757]')} style={{ width: `${pct}%` }} /></div><span className="text-xs text-[#8A8F98] w-16 text-right">{count} ({pct}%)</span></div>;
                })}</div>
              </div>
              ...
              {analysisResult.stats.low_reviews_sample?.length > 0 && (
                <div className="bg-[#08090A] rounded-xl p-4 border">
                  <h3 className="text-sm font-semibold text-[#F7F8F8] mb-3 flex items-center gap-1.5"><AlertTriangle size={14} className="text-[#EB5757]" /> 저별점 리뷰</h3>
                  <div className="space-y-2 max-h-64 overflow-y-auto">{analysisResult.stats.low_reviews_sample.map((r: any, idx: number) => (
                    <div key={idx} className="bg-[#0F1011] rounded-lg p-3 border text-xs">
                      <div className="flex items-center gap-2 mb-1"><Stars rating={r.rating} size={11} /><span className="text-[#8A8F98]">{r.writer}</span><span className="text-[#62666D] ml-auto">{r.date?.slice(0, 10)}</span></div>
                      <p className="text-[#D0D6E0] leading-relaxed">{r.content?.slice(0, 200)}</p>
                    </div>
                  ))}</div>
                </div>
              )}
    ...
    <div className="bg-[#0F1011] rounded-xl border border-[#23252A] p-5">
      ...
      {showForm && (
        <div className="p-4 bg-[#08090A] rounded-lg border space-y-4 mb-4">
          ...
          <div><label className="text-xs text-[#8A8F98] mb-1 block">이메일</label><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="report@example.com" className="w-full px-3 py-1.5 border rounded-lg text-sm" /></div>
          <div className="flex gap-2"><button onClick={() => create.mutate()} disabled={!email || days.length === 0 || create.isPending} className="px-4 py-1.5 bg-[#27A644] text-white text-sm font-medium rounded-lg hover:bg-green-700 disabled:opacity-50">{create.isPending ? '등록 중...' : '스케줄 등록'}</button><button onClick={() => setShowForm(false)} className="px-4 py-1.5 text-sm text-[#8A8F98] border rounded-lg hover:bg-[#141516]/5">취소</button></div>
        </div>
      )}
      {schedules.length > 0 ? <div className="space-y-2">{schedules.map((s: any) => (
        <div key={s.id} className="flex items-center justify-between p-3 bg-[#08090A] rounded-lg border text-xs">
          <div className="flex items-center gap-2 flex-wrap"><span className="font-medium text-[#F7F8F8]">{s.name}</span><span className="text-[#62666D]">|</span><span className="text-[#8A8F98]">{(s.days_of_week||[]).map((d:number)=>['일','월','화','수','목','금','토'][d]).join('·')} {String(s.send_hour??9).padStart(2,'0')}:{String(s.send_minute??0).padStart(2,'0')}</span><span className="text-[#62666D]">&rarr;</span><span className="text-[#27A644]">{s.email_to}</span><span className="px-1.5 py-0.5 bg-[#EB5757]/15 text-[#EB5757] rounded text-[10px]">{s.star_threshold}점↓</span></div>
          <div className="flex gap-1.5 ml-2"><button onClick={() => runNow.mutate(s.id)} disabled={runNow.isPending} className="p-1.5 bg-[#27A644]/10 text-[#27A644] rounded hover:bg-[#27A644]/15" title="즉시 실행">{runNow.isPending ? <Loader2 size={13} className="animate-spin" /> : <Star size={13} />}</button><button onClick={() => del.mutate(s.id)} className="p-1.5 bg-[#EB5757]/10 text-[#EB5757] rounded hover:bg-[#EB5757]/15" title="삭제"><Trash2 size={13} /></button></div>
        </div>
      ))}</div> : <p className="text-xs text-[#62666D] py-2">등록된 스케줄이 없습니다.</p>}
    </div>
  );
}
