...
function TrendChart({
  ...
  if (!data || data.length < 2) {
    return (
      <div
        className="w-full bg-bg-0 rounded-xl flex items-center justify-center text-text-quaternary text-sm"
        style={{ height }}
      >
        데이터 없음
      </div>
...
function ProductCard({ item, rank }: { item: ShoppingItem; rank: number }) {
  ...
  return (
    <div className="bg-bg-1 rounded-xl border border-border-primary overflow-hidden hover:shadow-[0px_3px_12px_rgba(0,0,0,0.2)] transition-shadow group">
      {/* Image area */}
      <div className="relative aspect-square bg-bg-2 overflow-hidden">
        {item.image ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <ShoppingBag size={40} className="text-text-quaternary" />
          </div>
        )}
        {/* Rank badge */}
        <div className="absolute top-2 left-2 w-7 h-7 rounded-full bg-green text-white text-xs font-bold flex items-center justify-center shadow">
          {rank}
        </div>
      </div>
      ...
      <div className="p-3 space-y-1">
        <p className="text-xs text-text-tertiary truncate">{item.mallName}</p>
        <p className="text-sm font-medium text-text-primary line-clamp-2 leading-tight min-h-[2.5rem]">
          {cleanTitle}
        </p>
...
export function NaverKeywordResearch() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      return data as { items: ShoppingItem[]; available?: boolean; error?: string; source?: string };
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-text-primary flex items-center gap-2">
            <Search className="text-green" size={28} />
            키워드 리서치
          </h1>
      ...
      <div className="bg-bg-1 rounded-xl border border-border-primary p-5">
        ...
        {registeredKeywords.length > 0 && (
          <div className="mt-3 flex items-center gap-2 flex-wrap">
            <span className="text-xs text-text-tertiary">등록 키워드:</span>
      ...
      {/* Empty state */}
      {!searchedKeyword && (
        <div className="bg-bg-1 rounded-xl border border-border-primary py-20 flex flex-col items-center gap-4 text-text-quaternary">
          <Search size={56} className="text-text-quaternary" />
        ...
        <>
          ...
          <div className="bg-bg-1 rounded-xl border border-border-primary p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
              <h2 className="text-base font-semibold text-text-primary flex items-center gap-2">
                ...
                검색량 트렌드
                <span className="ml-1 px-2 py-0.5 bg-green/10 text-green rounded text-xs font-medium border border-green/30">
                  {searchedKeyword}
                </span>
              </h2>
              ...
              <div className="flex items-center gap-3 flex-wrap gap-y-2">
                {/* Period */}
                <div className="flex items-center bg-bg-2 rounded-lg p-0.5 gap-0.5">
                  {PERIOD_OPTIONS.map((opt) => (
                    <button
                      ...
                      className={clsx(
                        ...
                        period === opt.value
                          ? 'bg-bg-1 shadow-[0px_1px_3px_rgba(0,0,0,0.2)] text-green'
            ...
            {volumeLoading ? (
              ...
              <>
                  ...
                  return (
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-5">
                      <div className="bg-gradient-to-br from-green/10 to-bg-1 border border-green/30 rounded-xl p-4 text-center">
                        ...
                        <p className="text-2xl font-bold text-green">{exact.monthlyTotalQcCnt.toLocaleString()}</p>
                      </div>
                      <div className="bg-gradient-to-br from-bg-0 to-bg-1 border border-brand/30 rounded-xl p-4 text-center">
                        <div className="flex items-center justify-center gap-1.5 text-xs text-text-tertiary mb-1">
                      ...
                      <div className="bg-gradient-to-br from-orange/10 to-bg-1 border border-orange/30 rounded-xl p-4 text-center">
                        ...
                        <p className="text-2xl font-bold text-orange">{exact.monthlyMobileQcCnt.toLocaleString()}</p>
                      </div>
                    </div>
                  );
                })()}
                ...
                {volumeData.data.length > 1 && (
                  <div>
                    <h3 className="text-sm font-semibold text-text-secondary mb-2">연관 키워드 검색량</h3>
                    <div className="overflow-x-auto rounded-lg border border-border-primary">
                      <table className="w-full text-sm min-w-[560px]">
                        <thead>
                          <tr className="bg-bg-0 text-text-tertiary text-xs">
                        ...
                        <tbody>
                          {volumeData.data.slice(0, 10).map((item, idx) => (
                            <tr key={idx} className={clsx('border-t border-border-primary', item.keyword === searchedKeyword && 'bg-green/10 font-semibold')}>
                              <td className="px-3 py-2 text-text-primary">{item.keyword}</td>
                              <td className="text-right px-3 py-2 text-text-secondary">{item.monthlyTotalQcCnt.toLocaleString()}</td>
                              <td className="text-right px-3 py-2 text-text-tertiary">{item.monthlyPcQcCnt.toLocaleString()}</td>
                              <td className="text-right px-3 py-2 text-text-tertiary">{item.monthlyMobileQcCnt.toLocaleString()}</td>
                              <td className="text-right px-3 py-2">
                                <span className={clsx('px-1.5 py-0.5 rounded text-xs', {
                                  'bg-red/15 text-red': item.compIdx === '높음',
                                  'bg-yellow/15 text-yellow': item.compIdx === '중간',
                                  'bg-green/15 text-green': item.compIdx === '낮음',
                                  'bg-bg-2 text-text-tertiary': !item.compIdx || !['높음', '중간', '낮음'].includes(item.compIdx),
                                })}>
                                  {item.compIdx || '-'}
                                </span>
                              </td>
              ...
              <div className="bg-bg-0 rounded-xl p-5 text-center text-sm text-text-tertiary">
                <BarChart3 size={28} className="text-text-quaternary mx-auto mb-2" />
          ...
          {shoppingItems.length > 0 && (
            <div className="bg-bg-1 rounded-xl border border-border-primary p-6">
              ...
              {aiError && !aiLoading && (
                <div className="flex items-center gap-2 py-4 text-red text-sm">
                  <AlertCircle size={16} />
                  {aiError}
                </div>
              )}
              ...
              {aiAnalysis && !aiLoading && (
                <div className="bg-bg-1 border border-border-primary rounded-xl overflow-hidden">
                  ...
                  <div className={clsx('px-6 py-5 relative', aiCollapsed && 'max-h-[400px] overflow-hidden')}>
                    ...
                    />
                    {aiCollapsed && (
                      <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-bg-1 to-transparent" />
                    )}
                  </div>
                  {/* Toggle button */}
                  <div className="px-6 py-3 border-t border-border-primary flex justify-center">
                    <button
          ...
          <div className="bg-bg-1 rounded-xl border border-border-primary p-6">
            ...
            {shoppingLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 gap-3 text-text-quaternary">
                ...
                <p className="text-sm font-medium text-text-secondary">쇼핑 데이터 수집 실패</p>
                <p className="text-xs text-center max-w-md">
                  {shoppingData.error || '쇼핑 데이터를 가져오지 못했습니다. 잠시 후 다시 시도해주세요.'}
                </p>
              </div>
            ) : shoppingError ? (
              ...
              <>
                {shoppingData?.source === 'serp_block' && (
                  <p className="text-xs text-text-quaternary mb-3 bg-bg-0 border border-border-primary rounded-lg px-3 py-2">
                    네이버 쇼핑 검색 API 종료로, 통합검색 결과의 <span className="font-medium text-text-secondary">쇼핑 블록 상위 노출 상품</span> 기준으로 표시됩니다. 사용자가 검색 시 실제로 보는 노출 순서입니다.
                  </p>
                )}
              ...
              <div className="flex flex-col items-center justify-center py-20 gap-3 text-text-quaternary">
                <ShoppingBag size={48} className="text-text-quaternary" />
...
function RankResultCard({ data }: { data: any }) {
  ...
  const rankBadge = (rank: number) => {
    const color = rank <= 10 ? 'text-green' : rank <= 30 ? 'text-yellow' : 'text-red';
    const bg = rank <= 10 ? 'bg-green/10' : rank <= 30 ? 'bg-yellow/10' : 'bg-red/10';
  ...
  return (
    <div className="rounded-lg border overflow-hidden">
      ...
      <button
        ...
        className={clsx('w-full flex items-center justify-between flex-wrap gap-y-1 px-4 py-3 text-left transition-colors', hasDetail ? 'hover:bg-bg-2/5 cursor-pointer' : 'cursor-default')}
      >
        <span className="text-sm font-semibold text-text-primary min-w-[100px]">{data.keyword}</span>
        <div className="flex items-center gap-6 flex-wrap gap-y-1">
          <div className="text-xs">
            <span className="text-text-tertiary mr-1.5">쇼핑블록</span>
            {shopUnavailable
              ? <span className="text-text-quaternary" title="통합검색 쇼핑 블록 수집 실패">수집 실패</span>
              : shopRanks.length > 0
                ? <>{rankBadge(shopRanks[0].rank)} <span className="text-text-quaternary">/ {data.shopping_total?.toLocaleString()}개 카드</span></>
                : (data.shopping_total ?? 0) === 0
                  ? <span className="text-text-quaternary" title="이 키워드 통합검색에는 쇼핑 블록이 없습니다">블록 없음</span>
                  : <span className="text-red font-medium">미노출</span>}
            {data.brand_chip && (
              <span className="ml-1.5 px-1.5 py-0.5 bg-green/10 text-green rounded text-[10px] border border-green/30" title="쇼핑 블록 브랜드 필터에 브랜드가 노출됩니다">브랜드필터</span>
            )}
          </div>
          <div className="text-xs">
            <span className="text-text-tertiary mr-1.5">블로그</span>
            {blogRanks.length > 0
              ? <>{rankBadge(blogRanks[0].rank)} <span className="text-text-quaternary">/ {data.blog_total?.toLocaleString()}건</span></>
              : <span className="text-red font-medium">미노출</span>}
          </div>
          {hasDetail && (
            <span className={clsx('text-text-quaternary transition-transform text-xs', expanded && 'rotate-180')}>▼</span>
          )}
        </div>
      </button>
      ...
      {/* 상세 펼침 */}
      {expanded && (
        <div className="border-t bg-bg-0 px-4 py-3 space-y-3">
          {/* 쇼핑 상세 */}
          {shopRanks.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-orange mb-1.5 flex items-center gap-1">
                <ShoppingBag size={12} /> 네이버 쇼핑 노출 제품 ({shopRanks.length}건)
              </p>
              <div className="space-y-1">
                {shopRanks.map((s: any, idx: number) => (
                  <div key={idx} className="flex items-center gap-2 bg-bg-1 rounded-lg px-3 py-2 border text-xs">
                    <span className={clsx('font-bold w-8 text-center', s.rank <= 10 ? 'text-green' : s.rank <= 30 ? 'text-yellow' : 'text-red')}>{s.rank}위</span>
                    <div className="flex-1 min-w-0">
                      <p className="text-text-primary truncate font-medium">{s.title}</p>
                      <div className="flex items-center gap-2 text-text-quaternary mt-0.5">
          ...
          {blogRanks.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-accent-hover mb-1.5 flex items-center gap-1">
                <FileText size={12} /> 네이버 블로그 노출 ({blogRanks.length}건)
              </p>
              <div className="space-y-1">
                {blogRanks.map((b: any, idx: number) => (
                  <div key={idx} className="flex items-center gap-2 bg-bg-1 rounded-lg px-3 py-2 border text-xs">
                    <span className={clsx('font-bold w-8 text-center', b.rank <= 10 ? 'text-green' : b.rank <= 30 ? 'text-yellow' : 'text-red')}>{b.rank}위</span>
                    <div className="flex-1 min-w-0">
                      <p className="text-text-primary truncate font-medium">{b.title}</p>
                      <div className="flex items-center gap-2 text-text-quaternary mt-0.5">
...
function AiRankAnalysis({ text }: { text: string }) {
  ...
  const boldify = (s: string) => s.replace(/\*\*(.+?)\*\*/g, '<strong class="text-text-primary">$1</strong>');
        ...
        {sections.map((sec, i) => {
          ...
          return (
            <div key={i} className={clsx('rounded-xl border p-4', style.border, style.bg)}>
              ...
              <div className="text-xs text-text-secondary leading-relaxed pl-0.5">
...
function KeywordRankMonitor({ brandName, registeredKeywords = [] }: { brandName: string; registeredKeywords: any[] }) {
  ...
  return (
    <div className="mt-8 rounded-xl border border-green/30 bg-bg-1 overflow-hidden">
      {/* Header */}
      <button
      ...
      {open && (
        <div className="px-5 pb-5 space-y-4 border-t border-green/30">
          ...
          {registeredKeywords.length === 0 && (
            <p className="text-xs text-yellow bg-yellow/10 p-2 rounded-lg">키워드가 등록되지 않았습니다. 위 검색바에서 키워드를 입력하고 [+ 등록] 버튼을 눌러주세요.</p>
          )}
          ...
          {rankResult?.rank_results && (
            <div className="space-y-3">
              ...
              <p className="text-xs text-text-quaternary bg-bg-0 border border-border-primary rounded-lg px-3 py-2">
                쇼핑 순위는 네이버 <span className="font-medium text-text-secondary">통합검색 쇼핑 블록</span> 노출 순위 기준입니다 (쇼핑검색 API 종료 대체 · 사용자가 검색 시 실제로 보는 영역).
                {rankResult.rank_results.some((r: any) => r.shopping_available === false) && (
                  <span className="text-yellow"> 일부 키워드는 쇼핑 데이터 수집에 실패했습니다.</span>
                )}
              </p>
          ...
          <div className="border-t border-green/30 pt-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setShowForm(!showForm)} className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-green bg-green/10 rounded-lg hover:bg-green/15 border border-green/30">
                <Plus size={13} /> 스케줄 추가
              </button>
            </div>

            {showForm && (
              <div className="p-4 bg-bg-0 rounded-lg space-y-4 mb-3 border">
                {/* 요일 선택 */}
                <div>
                  <label className="text-xs text-text-tertiary mb-2 block">발송 요일 (클릭하여 선택)</label>
                  ...
                  {schedDays.length === 0 && <p className="text-xs text-red mt-1">최소 1개 요일을 선택해주세요.</p>}
                </div>
                {/* 시간 입력 */}
                <div>
                  <label className="text-xs text-text-tertiary mb-2 block">발송 시간 (KST)</label>
                ...
                <div>
                  <label className="text-xs text-text-tertiary mb-1 block">수신 이메일</label>
            ...
            {schedules.length > 0 ? (
              <div className="space-y-2">
                {schedules.map((s: any) => (
                  <div key={s.id} className="flex items-center justify-between p-3 bg-bg-1 rounded-lg border text-xs">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-green">{s.email_to}</span>
                      {s.next_run_at && <span className="text-text-quaternary">(다음: {new Date(s.next_run_at).toLocaleString('ko-KR')})</span>}
                    </div>
                    <div className="flex gap-1.5 ml-2">
                      <button onClick={() => runNow.mutate(s.id)} disabled={runNow.isPending}
                        className="p-1.5 bg-green/10 text-green rounded hover:bg-green/15" title="즉시 실행"><Play size={13} /></button>
                      <button onClick={() => deleteSched.mutate(s.id)}
                        className="p-1.5 bg-red/10 text-red rounded hover:bg-red/15" title="삭제"><Trash2 size={13} /></button>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-xs text-text-quaternary py-2">등록된 스케줄이 없습니다. 스케줄을 추가하면 정해진 시간에 순위 리포트가 이메일로 발송됩니다.</p>
            )}
          </div>
        </div>
      )}
