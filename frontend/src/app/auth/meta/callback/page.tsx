...
function MetaCallbackContent() {
  ...
  return (
    <div className="bg-bg-1 rounded-2xl shadow-[0px_7px_32px_rgba(0,0,0,0.35)] max-w-md w-full p-8">
      {status === 'processing' && (
        <div className="text-center">
          <div className="w-12 h-12 border-4 border-brand border-t-transparent rounded-full animate-spin mx-auto mb-4" />
          <h2 className="text-xl font-semibold text-text-primary mb-2">Meta 계정 연결 중...</h2>
          <p className="text-text-tertiary">잠시만 기다려주세요.</p>
        </div>
      )}
      ...
      {status === 'select_account' && (
        <div>
          <div className="text-center mb-6">
            <div className="w-12 h-12 bg-blue/15 rounded-full flex items-center justify-center mx-auto mb-3">
              <span className="text-2xl">📊</span>
            </div>
            <h2 className="text-xl font-semibold text-text-primary">광고 계정 선택</h2>
            <p className="text-text-tertiary mt-1">{metaName}님, 사용할 광고 계정을 선택하세요.</p>
          </div>
          ...
          {pages.length > 0 && (
            <div className="mt-6 pt-4 border-t">
              <h3 className="text-sm font-medium text-text-secondary mb-2">연결된 페이지</h3>
              {pages.map((page) => (
      ...
      {status === 'success' && (
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-text-primary mb-2">Meta 연동 완료!</h2>
          <p className="text-text-tertiary">{metaName}님의 계정이 연결되었습니다.</p>
          <p className="text-sm text-text-quaternary mt-2">잠시 후 메인 페이지로 이동합니다...</p>
        </div>
      )}

      {status === 'error' && (
        <div className="text-center">
          <div className="w-16 h-16 bg-red/15 rounded-full flex items-center justify-center mx-auto mb-4">
            <svg className="w-8 h-8 text-red" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            </svg>
          </div>
          <h2 className="text-xl font-semibold text-text-primary mb-2">연결 실패</h2>
          <p className="text-red mb-4">{error}</p>
          <button
            onClick={() => router.push('/')}
            className="px-6 py-2 bg-bg-2 text-text-secondary rounded-lg hover:bg-bg-2/7 transition-colors"
          >
            돌아가기
          </button>
        </div>
...
export default function MetaCallbackPage() {
  return (
    <div className="min-h-screen bg-bg-0 flex items-center justify-center p-4">
      <Suspense
        fallback={
          <div className="bg-bg-1 rounded-2xl shadow-[0px_7px_32px_rgba(0,0,0,0.35)] max-w-md w-full p-8 text-center">
            <div className="w-12 h-12 border-4 border-brand border-t-transparent rounded-full animate-spin mx-auto mb-4" />
            <h2 className="text-xl font-semibold text-text-primary mb-2">로딩 중...</h2>
          </div>
        }
      >
        <MetaCallbackContent />
